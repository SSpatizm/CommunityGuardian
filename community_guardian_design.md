...
## 12. README Template (Pre-filled)

```markdown
# Community Guardian

A community safety incident reporting and intelligence platform with AI-powered classification.

## Candidate Name
[FILL]

## Scenario Chosen
#3 — Community Safety & Digital Wellness (Neighborhood Groups)
...
### Prerequisites
- Python 3.10+
- An Anthropic API key (optional — rule-based fallback works without it)
...
python seed.py              # Load synthetic data
...
- **Flask** — HTTP, forms, rendering. No business logic.
- **Classifier adapter** — AI-first with deterministic rule engine fallback. Pure function: text in, classification dict out.
- **SQLite** — Single-table persistence. Stores classified incidents. No participation in classification logic.

AI classification attempts the configured LLM API. On any failure (timeout, auth, rate limit, malformed response, invalid output), the system silently falls back to a keyword-based rule engine that produces the same output schema. The caller and the end user are unaware of which path executed unless they inspect the classification metadata.

## AI Disclosure
- **Did you use an AI assistant?** Yes — Claude was used for design discussion, code scaffolding, and architecture review.
- **How did you verify suggestions?** [FILL — e.g., manually reviewed all generated code, tested each path independently, verified rule engine outputs against expected classifications]
- **One example of a suggestion you rejected or changed:** [FILL]

## Tradeoffs & Prioritization
- **What did you cut?** Digest feature (AI summarization of recent incidents), user authentication, real-time notifications, location-based filtering with geocoding.
- **What would you build next?** Three-tier classification cascade (commercial LLM → fine-tuned lightweight model → rule engine), user accounts with notification preferences, geographic clustering of incidents on a map view, trend detection over time.
- **Known limitations:** No authentication (any user can create/edit any report). Location is free text, not geocoded. Rule engine keyword matching is English-only and does not handle synonyms or misspellings. AI classification requires network access and incurs API costs.
```

---

## 13. Video Script (5–7 minutes)

| Segment | Duration | Content |
|---------|----------|---------|
| Problem & Audience | 0:30 | "Neighborhood communities face alert fatigue — scattered incident data, no filtering, no actionable next steps. This mirrors the core challenge in security operations: noise-to-signal transformation." |
| Architecture | 1:00 | Walk through the three-layer diagram. Emphasize decoupling: Flask knows nothing about classification, classifier knows nothing about storage. Mention why SQLite over flat JSON. Mention rule engine as defense-in-depth — graceful degradation to deterministic behavior when AI fails. |
| Live Demo: Create | 1:30 | Create a new incident report. Show the form. Submit a phishing report. Show it appearing in the feed with correct classification. Show the detail view with classification metadata. |
| Live Demo: Filter & Search | 0:45 | Filter feed by category (cyber). Filter by severity (critical). Search for "phishing". Clear filters. |
| Live Demo: Edit | 0:30 | Edit the report, change description significantly. Show reclassification happens automatically. |
| Live Demo: AI Fallback | 1:00 | Temporarily set API key to invalid value. Create another report. Show it still classifies correctly via rule engine. Show the fallback metadata in the detail view. Reset API key. |
| Tests | 0:30 | Run pytest. Show tests passing. Briefly explain what each tests. |
| What's Next | 0:30 | Three-tier cascade, user auth, geographic clustering, trend detection. Reference Palo Alto's defense-in-depth philosophy. |
| Close | 0:15 | "Thank you for reviewing." |

**Total: ~6:30**

---

## 14. Implementation Order (Time Budget)

| Phase | Task | Estimate |
|-------|------|----------|
| 1 | `db.py` — schema, init_db(), get_db(), insert/select helpers | 20 min |
| 2 | `rules.json` — full keyword/action config | 15 min |
| 3 | `classifier.py` — rule engine + AI path + adapter | 60 min |
| 4 | `validators.py` — form validation | 10 min |
| 5 | `app.py` — all routes | 45 min |
| 6 | Templates — base, feed, form, detail | 45 min |
| 7 | `static/style.css` — minimal styling | 15 min |
| 8 | `sample_data.json` + `seed.py` | 10 min |
| 9 | `test_classifier.py` — 2+ tests | 15 min |
| 10 | `.env.example`, `.gitignore`, `requirements.txt` | 5 min |
| 11 | README — fill template | 15 min |
| 12 | End-to-end testing, bug fixes | 20 min |
| 13 | Video recording | 25 min |
| | **Total** | **~5 hours** |

Buffer: 1 hour for unexpected issues, debugging, and polish.

---

## 15. Success Metrics Alignment
